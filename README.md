...
- Отдельный версионируемый домен композиций со снимками слоёв.
- Раздельная история расчётного и фактического BCT.
- История стоимости композиции в рублях за м².
...
Полный импорт/инвентаризация, связь выбранного раскроя с потребностью, аутентификация и складской интерфейс добавляются следующими срезами v0.9.
...
- `POST /api/v1/compositions` — новая композиция и версия 1.
- `POST /api/v1/compositions/{id}/versions` — следующая неизменяемая версия.
